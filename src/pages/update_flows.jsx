/**
 * @file            src/pages/update_flows.jsx
 * @project         DocFlow
...
export default function UpdateFlows({ setCurrentPage, flowId }) {
    ...
    useEffect(() => {
        ...
        const loadFlow = async () => {
            try {
                const res = await fetch(`/api/flows.php?id=${flowId}`); // fetch the data of the flow with its ID
                const data = await res.json();
                if (res.ok) {
                    setForm({
                        ...data,
                        auto_trigger: !!data.auto_trigger, // react shortcut to convert an integer into a boolean (sqlite returns 0/1, but the form expects true/false)
                        convert_docx: !!data.convert_docx,
                        convert_xlsx: !!data.convert_xlsx,
                    });
                }
                } catch (err) {
        ...
        loadFlow();
    }, [flowId]); // executes the instruction block when the page is built, and when flowId is modified
    ...
    return (
        <div className="w-full bg-neutral-200 text-black flex flex-col p-4">
            <div className="max-w-xl w-full mx-auto mt-5 bg-white rounded-2xl shadow-md p-8 flex flex-col gap-6">
                <h1 className="text-2xl font-semibold mb-4">Éditer le Flow</h1>
